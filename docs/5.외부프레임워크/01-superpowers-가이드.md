...
| 상황 | 사용할 스킬 |
|---|---|
| **신규 기능 개발 시** | `brainstorming` — 요구사항·설계를 정제한 뒤 코드에 손댐 |
| **큰 일을 서브에이전트한테 나눠서 시킬 때** | `subagent-driven-development` — 병렬 에이전트로 작업 분배 |

나머지 스킬(TDD, code-review, worktrees 등)은 위 두 흐름 안에서 자동으로 따라오거나 필요할 때만 호출하면 된다.
